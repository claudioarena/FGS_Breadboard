...
def HeNeAr_fit(calimage, linelist='apohenear.dat', interac=True,
               ...
               second_pass=True, disp_approx = 0., wcen_approx = 1., datasec=[0, 0, 0, 0]):
    """
    ...
    if trim is False:
        if datasec is [0, 0, 0, 0]:
            img = hdu[0].data
        else:
            img = hdu[0].data[datasec[2] - 1:datasec[3], datasec[0] - 1:datasec[1]]
    ...
    try:
        ...
        disp_approx = disp_approx
        wcen_approx = wcen_approx
            ...
            class InteracWave(object):
                ...
                def OnClick(self, event):
                    ...
                    if self.fig.canvas.manager.toolbar._active is None:
                        ...
                        #print("Clicked")
                        #print(ix)
                        #print(np.nanmin(slice))
                        #print(np.nanmax(slice))
                        ...
                        if (ix is not None) and (ix > np.min(wtemp)) and (ix < np.max(wtemp)) :
                            ...
                            if (len(nearby[0]) > 4) and (kill is None):
                                ...
                                try:
                                    ...
                                    self.ax.plot(wtemp[int(popt[2])], slice[int(wtemp[int(popt[2])])], 'r|')
                                except ValueError:
                                ...
                                try:
                                    ...
                                    self.ax.plot(wtemp[int(popt[2])], slice[int(wtemp[int(popt[2])])], 'ro')
